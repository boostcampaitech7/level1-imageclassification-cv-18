...
if __name__ == "__main__":
    ...
    # path 설정
    datapath = "/data/ephemeral/home/taehan/level1-imageclassification-cv-18/last_train/eva_large_mlp/Experiments/eva_large_mlp/test_csv" # 변경 필요 : 데이터 위치를 npy가 있는 위치로 변경 해야함
    ...
    for fold in range(k_folds):
        predictions = np.load(os.path.join(datapath, f'fold{fold+1}_eva_large_mlp.npy')) # 변경 필요 : 자신의 npy파일명으로 변경 해야함

        # 라벨 예측
        final_predictions = np.argmax(predictions, axis=1)

        csv_name_fold = f"hard_fold{fold+1}_eva_large_mlp.csv" # 변경 필요 : 자신이 원하는 csv파일명으로 변경 해야함
