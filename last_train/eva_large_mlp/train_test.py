...
def train_test():
    ...
    for fold in range(k_folds):
        ...
        np.save(os.path.join(test_csv_dir, f"fold{fold+1}_eva_large_mlp.npy"), predictions)
        
        csv_name_fold = f"fold{fold+1}_eva_large_mlp.npy.csv"
    ...
    np.save(os.path.join(test_csv_dir, f"all_fold_eva_large_mlp.npy"), k_fold_predictions)
    ...
    np.save(os.path.join(test_csv_dir, f"softvoting_all_fold_eva_large_mlp.npy"), final_predictions)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_rootpath', type=str, default="Experiments/eva_large_mlp", help='가중치, log, tensorboard 그래프 저장을 위한 path 실험명으로 디렉토리 구성')
