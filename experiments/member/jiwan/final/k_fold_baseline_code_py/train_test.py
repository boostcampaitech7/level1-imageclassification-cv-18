...
def train_test():
    ...
    for fold in range(k_folds):
        ...
        np.save(os.path.join(test_csv_dir, f"fold{fold+1}_eva_large_head.npy"), predictions)
        
        # 각 fold 라벨 예측
        fold_final_predictions = np.argmax(predictions, axis=1)

        csv_name_fold = f"fold{fold+1}_eva_large_head.csv"
        result_info = test_info.copy()
        result_info['target'] = fold_final_predictions 
    ...
    np.save(os.path.join(test_csv_dir, f"all_fold_eva_large_head.npy"), k_fold_predictions)
    ...
    np.save(os.path.join(test_csv_dir, f"softvoting_all_fold_eva_large_head.npy"), final_predictions)

    # test_info의 복사본을 사용하여 CSV 저장
    csv_name = "softvoting_5_eva_large_head.csv"
    result_info = test_info.copy()
