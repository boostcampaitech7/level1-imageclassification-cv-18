...
def train_test():
    ...
    for fold in range(k_folds):
        ...
        np.save(os.path.join(test_csv_dir, f"fold{fold+1}_eva_large_curriculum_head.npy"), predictions)
        
        csv_name_fold = f"fold{fold+1}_eva_large_curriculum_head.csv"
    ...
    np.save(os.path.join(test_csv_dir, f"all_fold_eva_large_curriculum_head.npy"), k_fold_predictions)
    ...
    np.save(os.path.join(test_csv_dir, f"softvoting_all_fold_eva_large_curriculum_head.npy"), final_predictions)

    # test_info의 복사본을 사용하여 CSV 저장
    csv_name = "softvoting_5_eva_large_curriculum_head.csv"
    result_info = test_info.copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_rootpath', type=str, default="Experiments/eva_large_curriculum_head", help='가중치, log, tensorboard 그래프 저장을 위한 path 실험명으로 디렉토리 구성')
